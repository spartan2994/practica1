...
<header>
 ...
 <li><a href="http://bogoduba.webcindario.com/producto.php">BoGoDuBa</a></li>
     ...
     <div class="search_bar">
       ...
       <input name= "BUSCAR" type="text" id="bar" placeholder="Buscar producto...">
       </form>
     </div>
    </nav>
</header>
...
<?php 		
			 ...
			 $idj=$_POST['codigo'];
			  ...
			  $consulta2=@mysql_query("UPDATE `productos` SET `nombre`='$cod_g',`descripcion_prod`='$d_g',`precio`='$min',`existencia`='$idj' WHERE id_producto='$cod_p'");
...
<?php 						 
			 ...
			 $consulta3=@mysql_query("INSERT INTO `bogoduba`.`productos`(`id_producto`, `nombre`, `descripcion_prod`, `precio`, `existencia`) VALUES ('$nomb5','$fech5','$pos5','$codigo_e5','$codigo_ju5')");
			 ...
			 $consulta4=@mysql_query("DELETE  FROM `productos` WHERE id_producto='$nomb4'");
			 @mysql_close($link);
	...
	$fecha_nac="";
	    ...
	    $consulta=@mysql_query("select * from productos where nombre like '%".$_POST['BUSCAR']. "%' or descripcion_prod like '%".$_POST['BUSCAR']. "%'");
		...
		$nombre=$filas['id_producto'];
		$fecha_nac=$filas['nombre'];
		$posicion=$filas['descripcion_prod'];
		$codigo=$filas['precio'];
		$codigo_j=$filas['existencia'];
